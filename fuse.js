...
const aiHtml = `
            ...
            <div class="card p-6" style="border-left: 4px solid #9c27b0;">
                <h2 class="text-xl font-bold mb-4" style="color: #e0e0e0;">🧠 Análisis Predictivo (IA Concurrente)</h2>
...
// Insert the AI HTML right after the control grid
content = content.replace(
    '<div class="grid dashboard-grid mb-6">',
    '<div class="grid dashboard-grid mb-6">\n' + aiHtml
);
...
const workerJs = `
        ...
        aiWorker.onmessage = function(e) {
            ...
            if (action === 'PREDICTION_RESULT') {
                document.getElementById('prob-home').textContent = (prediction.winProbHome * 100).toFixed(1) + '%';
                document.getElementById('bar-home').style.width = (prediction.winProbHome * 100) + '%';
                
                document.getElementById('prob-away').textContent = (prediction.winProbAway * 100).toFixed(1) + '%';
...
// To make sure it runs the UI update independently of the separate css/js 
